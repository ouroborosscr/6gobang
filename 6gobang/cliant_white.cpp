...
cliant_white::cliant_white(QWidget *parent) :
    QWidget(parent),
    ui(new Ui::cliant_white)
{
    ui->setupUi(this);
...
//                    connect(white55_win,SIGNAL(ExitWin()),my_open,SLOT(show()));
    ...
    connect(my_connect,&QPushButton::clicked,this,&cliant_white::myconnect);
...
//背景图片以及画棋盘的线和点
void cliant_white::paintEvent(QPaintEvent *)
{
    QPainter painter( this);

    connect(exit,&QPushButton::clicked,this,&cliant_white::close);
...
void cliant_white::showEvent(QShowEvent* )
{
    for(int i=1;i<=21;i++)
        for(int j=1;j<=21;j++)
        {
...
void cliant_white::closeEvent(QCloseEvent *)
{
    myclose();
    emit ExitWin();
}
...
//写一个鼠标事件，判定，发送数据
void cliant_white::mousePressEvent(QMouseEvent *event)
{
    myjudgement judge;
    warning44 warn44;
    warnin55 warn55;
...
//    qDebug()<<x1<<" "<<y1;
  ...
  //写一个条件，step为2的倍数+1，点为空，点击点在棋盘内（未完成）
...
cliant_white::~cliant_white()
{
    delete ui;
}


void cliant_white::myconnect()
{
    //获取服务器ip，端口
...
void cliant_white::send(int x,int y)
{
    if(NULL==tcpSocket)
    {
        return;
